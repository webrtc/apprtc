...
class DeclinePageHandlerTest(test_utilities.BasePageHandlerTest):
  ...
  def testDeclineRoomNotFound(self):
    ...
    room_id = 'callercallee'
    self.requestDeclineAndVerify(
        room_id, 'callee1gcm1', constants.RESPONSE_INVALID_ROOM)

  def testDeclineInvalidCallee(self):
    self.addTestData()
    room_id = 'callercallee'
    self.requestCallAndVerify(
        room_id, 'caller1gcm1', 'callee4', constants.RESPONSE_SUCCESS)
    # Wrong callee for room.
    self.requestDeclineAndVerify(
        room_id, 'callee2gcm1', constants.RESPONSE_INVALID_CALLEE)
    ...
    self.requestDeclineAndVerify(
        room_id, 'callee4gcm2', constants.RESPONSE_SUCCESS)
  ...
  def testDeclineAcceptedRoom(self):
    ...
    self.requestDeclineAndVerify(
        room_id, 'callee1gcm2', constants.RESPONSE_INVALID_ROOM)
  ...
  def testDeclineByCaller(self):
    ...
    self.requestDeclineAndVerify(
        room_id, 'caller1gcm1', constants.RESPONSE_INVALID_CALLEE)
