...
class AckPage(webapp2.RequestHandler):
  ...
  def handle_ringing(self, msg):
    ...
    if room_state != room.Room.STATE_WAITING:
      logging.warning('Room ' + room_id + ' is not in WAITING state ' +
    ...
    if not caller_gcm_id:
      logging.warning('Could not find caller gcm id when trying to'
  ...
  def post(self):
    ...
    if self.request.body:
      ...
      if util.has_msg_field(msg, constants.PARAM_TYPE, basestring):
        ...
        if ack_type == constants.TYPE_RINGING:
          self.handle_ringing(msg)
          return
